...
public class NotesPageController {
    ...
    public ResponseEntity<String> page() {
        ...
        sb.append(".act[disabled]{opacity:0.5;pointer-events:none}");
        ...
        sb.append("<a href='/notes.html' class=active>📝 Notes</a>");
        ...
        if (notes.isEmpty()) {
            ...
            sb.append("<div id=empty-q class=empty hidden>No notes match your filter.</div>");
            ...
            sb.append("const toast=document.getElementById('toast');");
            sb.append("function showToast(msg,err){toast.textContent=msg;toast.className=err?'show err':'show';");
            sb.append("clearTimeout(toast._t);toast._t=setTimeout(()=>toast.classList.remove('show','err'),2200);}");
            sb.append("function refilter(){const v=q.value.trim().toLowerCase();let visible=0;");
            sb.append("document.querySelectorAll('[data-note]').forEach(el=>{");
            sb.append("const hide=v!==''&&el.dataset.body.indexOf(v)<0;");
            sb.append("el.classList.toggle('hide',hide);if(!hide)visible++;});");
            sb.append("emptyEl.hidden=!(v!==''&&visible===0);}");
            sb.append("q.addEventListener('input',refilter);");
            ...
            sb.append("if(!id){showToast('Missing id',true);return;}");
            sb.append("b.disabled=true;");
            sb.append("try{");
            sb.append("if(b.dataset.act==='del'){");
            sb.append("const r=await fetch('/api/notes/'+encodeURIComponent(id),{method:'DELETE'});");
            sb.append("if(!r.ok)throw new Error('HTTP '+r.status);");
            sb.append("card.style.transition='opacity 0.15s';card.style.opacity='0';");
            sb.append("setTimeout(()=>{card.remove();refilter();},160);");
            sb.append("showToast('Deleted (recoverable from trash)');");
            sb.append("}else if(b.dataset.act==='pin'){");
            sb.append("const r=await fetch('/api/notes/'+encodeURIComponent(id)+'/pin',{method:'POST'});");
            sb.append("if(!r.ok)throw new Error('HTTP '+r.status);");
            sb.append("showToast('Pinned — reloading…');");
            sb.append("setTimeout(()=>location.reload(),300);");
            sb.append("}");
            sb.append("}catch(err){showToast('Failed: '+err.message,true);b.disabled=false;}");
            sb.append("});");
            sb.append("</script>");
        }
        sb.append("</body></html>");
