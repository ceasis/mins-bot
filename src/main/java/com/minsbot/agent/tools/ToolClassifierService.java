...
public class ToolClassifierService {
    ...
    private static final String SYSTEM_PROMPT = """
            ...
            - watcher: PRODUCT stock / size / restock / price-drop watching on shopping sites. Use THIS CATEGORY (not browser, not communication, not scheduling, not calendar) for ANY request like: 'notify me when [product] is in stock', 'alert me when [shoe] is available in size X', 'watch Nike/Shopee/Lazada/Zalora for restock', 'tell me when [item] comes back', 'check if size 9.5 is available', 'email me when price drops to X', 'watch for price drop on Y', 'Jordan 11 Gamma size 9.5', 'PS5 in stock alert', 'sneaker drop alert', 'restock notification'. The bot has WatcherTools.createWatcher that understands size availability, price ceilings, and sends real email or Discord/ntfy webhook notifications — purpose-built for this exact use case.
            ...
            - github: github, repository, repo, create repo, new repo, create a repository, new repository, create github project, new github project, make me a github repo, spin up a repo, init a github repo, pull request, PR, issue, commit, branch, gist, CI, workflow, actions, git, github notifications, github activity, merge, fork, stars, github search
            ...
            - screen_watcher: tell me when X appears on my screen, alert me when upload hits 100, ping me when CI turns green, notify me when this page changes, watch this area of my screen, wake me when the status badge changes, watch the progress bar, watch the build finish, monitor this region. PREFER THIS CATEGORY for region-based visual watching (different from watcher which is URL-based).
            ...
            - code_gen: create/scaffold/generate/build a new project or codebase — AND — continue/resume/modify/rewrite/port/convert/migrate/refactor/add-to an existing past project — AND — load/open/run/test/deploy a previously-created project. Covers: 'create me a java/python/react project', 'scaffold an app', 'build me a codebase', 'create me special/local <lang> project', 'continue <name>', 'work on <name>', 'add <feature> to <name>', 'modify <name>', 'resume <name>', 'rewrite <name> using <stack>', 'rewrite it using java/python/react/...', 'convert <name> to <stack>', 'port <name> to <lang>', 'migrate <name> to <framework>', 'reimplement <name> in <lang>', 'change stack of <name>', 'redo <name> with <stack>', 'load <name>', 'open <name> project', 'run <name>', 'start <name>', 'test <name>', 'where is my <name>', 'list my projects', 'show my generated projects'. **PREFER this category whenever the user references code, a project name, a stack change, or any create/edit/run of a codebase** — even if phrased casually ('run it', 'fix it', 'add X to it', 'rewrite it'). Tools: ClaudeCodeTools (primary create), SpecialCodeGenerator (user says 'special'), LocalCodeGenerator (user says 'local'), ContinueProjectTools (continue/modify/list past projects), DevServerTools (start/stop/tail-log a dev server without blocking — ALWAYS use this instead of runPowerShell for mvn spring-boot:run, npm run dev, flask run, etc).
